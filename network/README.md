...
Public URL: `https://claude.example.com`
...
- Tunnel config (routes `claude.example.com` → `localhost:4080`)
...
curl -s https://claude.example.com/health
```

Visit `https://claude.example.com` — Cloudflare Access will show a login
...
| `CORS_ORIGIN` | `*` | Allowed CORS origin. Set to `https://claude.example.com` in production |
