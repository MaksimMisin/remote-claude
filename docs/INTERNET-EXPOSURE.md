...
- DNS: `myhouse.example.com` → CNAME → `<tunnel-id>.cfargotunnel.com`
...
```typescript
const ALLOWED_ORIGIN = process.env.CORS_ORIGIN || 'https://claude.example.com';
...
- `cloudflare_zero_trust_tunnel_cloudflared_config` -- ingress: `claude.example.com` → `http://localhost:4080`
- `cloudflare_record` -- CNAME for `claude` subdomain
- `cloudflare_zero_trust_access_application` -- Access app for `claude.example.com`
...
2. Set up Cloudflare Tunnel for `claude.example.com`
